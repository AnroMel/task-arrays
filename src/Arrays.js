/*
  ...
  Преобразуете строку в массив чисел, отсортируйте их по возрастанию и верните строку с упорядоченными числами,
  разделенными запятыми.
*/
export function arraySorting(data) {
    return data
        .split(',') // Преобразуем строку в массив
        .map(Number) // Превращаем каждый элемент в число
...
export function arrayFiltering(data) {
    return data.filter((num) => num <= 100); // Оставляем только числа ≤ 100
}
...
export function arrayPushing(array1, array2) {
    ...
    for (let i = 0; i < array1.length; i++) {
        result.push(array1[i], array2[i]); // Чередуем элементы
    }
